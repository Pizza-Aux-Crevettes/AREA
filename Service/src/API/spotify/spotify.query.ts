...
export async function getRefreshSpotifyToken(email: string): Promise<any> {
    try {
        ...
        console.error('getRefreshSpotifyToken', e);
        return "";
    }
}

export async function updateSpotifyToken(email: string, access_token: string): Promise<boolean> {
    try {
        ...
        console.error('updateSpotifyToken', error);
        return false;
    }
}